...
import bert_data_module.transforms as t
import numpy as np
...
def tokenize_row(row):
    ...
    tokens = [tokenizer.cls_token] + row['post_tokens']
    rationale = [0] + row['rationale']

    tokens = [token.replace('<', '[').replace('>', ']') for token in tokens]

    token_ids = tokenizer.convert_tokens_to_ids(tokens)

    # for id_w, w in enumerate(tokens):
    #     buff = w
    #     if "<" in buff:
    #         buff = buff.replace("<", "")
    #     if ">" in buff:
    #         buff = buff.replace(">", "")
    #
    #     t = tokenizer(buff, add_special_tokens=False)
    #     token_ids += ids
    #
    #     if rationale[id_w] == 1:
    #         rat += [1] * len(ids)
    ...
    return {"bert_ids": token_ids,
            "bert_post_tokens": tokenizer.decode(token_ids),
            "bert_rationale": rationale}
...
def _reformat_csv(dataset: datasets.Dataset, split):
    ...
    # pretrain tokenizer does not recognize <user>
    bert_tokens = [[token.replace('<', '').replace('>', '') for token in token_seq] for token_seq in
                   df['post_tokens'].tolist()]
    bert_tokens = [[tokenizer.tokenize(token) for token in token_seq] for token_seq in bert_tokens]

    # align rationale with new tokens
    rationale = df['rationale'].tolist()
    bert_rationale = [[[r] * len(sub_tokens) for sub_tokens, r in zip(token_seq, rationale_seq)] for
                      token_seq, rationale_seq in zip(bert_tokens, rationale)]

    # flatten sublist
    bert_rationale = [[r for r_subword in rationale_seq for r in r_subword] for rationale_seq in bert_rationale]
    bert_tokens = [[subtok for subtok_list in token_seq for subtok in subtok_list] for token_seq in bert_tokens]

    # convert to ids
    bert_ids = [tokenizer.encode(token_seq) for token_seq in bert_tokens]  # auto add CLS and SEP
    # Add CLS and SEP
    bert_rationale = [[0] + rationale_seq + [0] for rationale_seq in bert_rationale]
    # for debug
    bert_post_tokens = tokenizer.batch_decode(bert_ids)
    ...
    df["bert_ids"] = pd.Series(bert_ids)
    df["bert_post_tokens"] = pd.Series(bert_post_tokens)
    df["bert_rationale"] = pd.Series(bert_rationale)
